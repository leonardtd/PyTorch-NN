...
class IsotropicVIG(nn.Module):
    def __init__(self, in_channels, num_blocks, kernel_size=16):
        super().__init__()

        self.num_blocks = num_blocks

        self.patchifier = Patchifier(
            patch_size=4, hidden_channels=in_channels, norm=True
        )
...
def test_iso_vig():
    # Tiny config
    model = IsotropicVIG(in_channels=192, num_blocks=12, kernel_size=16)
